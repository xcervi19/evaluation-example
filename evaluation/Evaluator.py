...
def get_profit_long(x, position_params):
    if x[0] == x[1]:
...
def add_profit_along_time(pair_timeseries, position_params):
    ...
    tp_arr = np.apply_along_axis(lambda x: get_tp(x), 1, open_to_high).reshape(open_to_high.shape[0],1)
    sl_arr = np.apply_along_axis(lambda x: get_sl(x), 1, open_to_low).reshape(open_to_low.shape[0],1)
    profit = np.apply_along_axis(lambda x: get_profit_long(x, position_params), 1, np.concatenate([tp_arr, sl_arr, open_tail_price], axis=1))
class Evaluator:
